...
def read_bag_postion(filepath):
    ...
    for topic, msg, t in bag.read_messages(topics='/novatel/oem7/odom'):
        position.append([msg.pose.pose.position.x, msg.pose.pose.position.y,
                        msg.pose.pose.position.z, msg.header.stamp.secs + msg.header.stamp.nsecs * 1e-9])
    # Normalize the odom message
    x0, y0, z0, _ = position[0]
    norm_position = []
    for i, pose in enumerate(position):
        norm_position.append(
            [pose[0] - x0, pose[1] - y0, pose[2] - z0, pose[3]])
    ...
    df2 = pd.DataFrame(imu, columns=['linear_acc_x', 'linear_acc_y', 'linear_acc_z',
                       'angular_vel_x', 'angular_vel_y', 'angular_vel_z', 'IMU_t'])
    file = pd.concat([df1, df2], axis=1)
    # print(f"{file_path}/{filename}.csv")
    file.to_csv(f'{file_path}/{filename}.csv', index=False)


if __name__ == '__main__':
    # fp = 'RTK_DATA/LiDAR_RTK/2023-03-06-12-12-06_1.bag'
